...
Inside you see a remote control."
      ...
      await keypress();
      game();
      //special text outcome for the rabbit hole (before the carrot given to the rabbit event happened)
...
slams you to the ground..."
      ...
      await keypress();
      game();
      //special outcome that checks the code for the safe, if it hasn't been opened yet
...
let table = new RoomObject("table", "It's a small end table.");
...
let hole = new RoomObject("hole", "It's the rabbit's hole.");
...
class Item {
  ...
  async use() {
    ...
    if (this.shortName === "package") {
      ...
      //long text for reading the letter inside the package
...
//With some fancy ASCII art text - found a generator online
...
${locationLookUp[locationCurrent].roomInventory}\n`);
  ...
  log(
    `Doors you can [go] through:${locationLookUp[locationCurrent].doorsInRoom}\n`
  );
